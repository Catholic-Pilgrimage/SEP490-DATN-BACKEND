...
class PlannerChatService {

    /**
     * Check if user can access planner chat (owner or member)
     */
    static async canAccessChat(plannerId, userId) {
     ...
     */
    static async getMessages(plannerId, userId, filters = {}) {
        try {
            // Check access
            const canAccess = await this.canAccessChat(plannerId, userId);
            if (!canAccess) {
     ...
     */
    static async sendMessage(plannerId, userId, messageData) {
        try {
     ...
     */
    static async uploadImage(plannerId, userId, file) {
        try {
            // Check access
            const canAccess = await this.canAccessChat(plannerId, userId);
            if (!canAccess) {
     ...
     * Delete a message
     */
    static async deleteMessage(plannerId, userId, messageId) {
        try {
     ...
     */
    static async clearMessages(plannerId) {
        try {
...
module.exports = PlannerChatService;
